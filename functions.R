...
# F_t
response.func <- function(climate, ecosystem, gamma = 0.02, alpha=1000, beta=1, disequil_abs = FALSE, disequil_sqr = FALSE, form=1, Fmax=NA){
  ...
  # with nonlinear equilibrium climate sensitivity
  if(form == 1000) response <- alpha + Fmax * (1-exp(-beta*climate)) # null nonlinear, exponential decay model - no effect of disequilibrium - same as Huxman et al. 2004
  if(form == 1001) response <- alpha + Fmax * (1-exp(-beta*climate + gamma*disequil*climate)) # nonlinear at equilibrium, disequilibrium affects slope
  if(form == 1002) response <- alpha + Fmax * (1-exp(-beta*climate + gamma*disequil)) # nonlinear at equilibrium, disequilibrium affects something??
  if(form == 1003) response <- alpha + Fmax * (1-exp(-beta*climate)) - gamma*disequil # nonlinear at equilibrium, disequilibrium affects intercept
...
acclimation.response.func <- function(time, clim_assym, clim_slope, clim_0, E0,
                                      ...
                                      legend_pos = "topleft", legend_cex = 0.75, Fmax=NA){
  ...
  response <- response.func(climate, ecosystem, gamma=gamma, alpha, beta, disequil_abs = disequil_abs, disequil_sqr = disequil_sqr, form=form, Fmax=Fmax)
  if(plot){
    par(mar=c(3,4,1,4))
...
fit.acc.model.sim <- function(F_t, C_t, t,
                              ...
                              disequil_abs = FALSE, disequil_sqr = FALSE, stepwise=FALSE, Fmax=NA){
