...
% DATE:     December 2025
% PURPOSE:  Plot time series for formula F5, assuming linearly increasing climate
...
ylim([0 25])
xlabel('time') 
ylabel({'Linear dependence'; 'form'; 'Ecosystem function (F_5)'}) 
title('a') 
...
for ii = 1:length(Delta0)
    ...
    E = C - r*lambda + (r*lambda - Delta0(ii))*exp(-tvals/lambda);
    %%% Compute time series for ecosystem function
    F(ii,:) = alpha + beta*(1-exp(-C*kappa))+ eta*E - gamma.*C.*(E-C).^2;
    plot(tvals,F(ii,:),'-','LineWidth',1.5,'color',Colors{ii})
end
xlim([0 max(tvals)]); set(gca,'XTick',[0 4 8])
ylim([-10 15])
xlabel('time') 
ylabel({'Nonlinear dependence'; 'form'; 'Ecosystem function (F_5)'}) 
title('d') 
set(gca,'Box','on', 'Layer','top','LineWidth', 1)
set(gca,'ycolor','k','FontSize',textsize,'FontWeight','Bold')
...
%%% Define parameters
% Uses same parameters as Panel C
